...
public class SearchGA extends Search 
{
	 ...
	 * @param genotypeA Genotype from first parent.
	 * @param genotypeB Genotype from second parent.
	 * @param xType Crossover type (0=uniform, 1=single-point, 2=double-point).
	 * @return Bits resulting from crossover.
	 */
	public long crossOver(final long genotypeA, final long genotypeB, final int xType)
	{
		long mask = 0;
		...
		case 2:  // Double-point crossover
			mask = (1L << (rng.nextInt() % domain.numBits())) ^ (1L << (rng.nextInt() % domain.numBits()));
			break;
		default:
