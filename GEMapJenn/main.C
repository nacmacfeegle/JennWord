...
void GlutManager::init_callbacks ()
{
    ...
    // Miguel ->
    //Menus::RootMenu::open();
    Menus::RootMenu::open(global_argc, global_argv);
    // <- Miguel
}
...
Options:\n\
    ...
    -w w1 w2 w3 w4              Define the vertex weights, e.g. 3 2 2 1\n\
    -s width height             Set initial window size\n\
    -dump filename              Save image to filename and quit\n\
    -z zoomlevel                Set initial zoom level\n\
    -h, --help                  Display this message\n\
see notes.text for complete examples of command-line arguments\n";
// <- Miguel
...
int main(int argc,char **argv)
{
    ...
    if (argc > 1) {
        ...
        for (; i<argc; ++i) {
            ...
            //activate dump mode
            if (arg == _dump) {
                Assert (i+1 < argc, "dump option requires a file name");
                i ++;
                fileToDump = argv[i];
                continue;
            }
            ...
            //set initial zoom
            if (arg == _z) {
                Assert (i+1 < argc, "zoom option requires a value");
                i ++;
