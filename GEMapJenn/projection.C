...
void Projector::capture (unsigned Nwide, unsigned Nhigh, std::string fileToDump)
// <- Miguel
{//captures, currently only grayscale
    ...
    // Miguel ->
    if(!fileToDump.size()){
        fileToDump = "jenn_capture.png";
    }
    //logger.info() << "opening file: jenn_capture.png" |0;
    logger.info() << "opening file: " << fileToDump |0;
    //FILE *file = fopen("jenn_capture.png", "wb");
    FILE *file = fopen(fileToDump.c_str(), "wb");
    // <- Miguel
    if (not file) {
